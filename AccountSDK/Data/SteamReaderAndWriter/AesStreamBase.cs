...
namespace Habby.Tool.IO
{
    public class AesStreamBase : System.IDisposable
    {
        protected const string AESKey = "fje)icl(*458$dHc81k5!@@ikldUo@lok#^dJFQAsddfDc<KH&jlOPI^WKDF@@$%@!JL2";
        protected const string AesTag = "tool";
        ...
        public static void EnCryptFile(string sourceFileName)
        {
            ...
            {
                Debug.LogError("尝试对已经加密的文件进行2次加密.sourceFileName = " + sourceFileName);
                tsourcefile.Dispose();
                return;
            }
        ...
        public static void DeCryptFile(string sourceFileName)
        {
            ...
            {
                treader.Dispose();
                Debug.LogError("试图解密一个非加密文件或文件无法解密.");
                return;
            }
